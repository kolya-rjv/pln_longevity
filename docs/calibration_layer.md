...
It is marked `STUB` and is **untested against a live Hyperon runtime** (no
`hyperon` in the env yet). The point of v0 is **not** to be numerically right —
it is to (a) lock the architecture and (b) make the clock atoms
